...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterLink, ButtonComponent, ImageGalleryComponent, LoadingSpinnerComponent, CurrencyBrlPipe, TruncatePipe],
  template: `
    <div class="space-y-6" *ngIf="dish(); else loading">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="page-title">{{ dish()?.name }}</h1>
          <p class="page-subtext">{{ dish()?.categoryName }}</p>
        </div>
        <div class="flex gap-3">
          <app-button variant="primary" [routerLink]="['/dishes', dish()?.id, 'edit']">
        ...
        <div class="space-y-6">
          <div class="card p-6">
            <div class="text-center">
              ...
              <p class="text-3xl font-bold text-gray-900">{{ dish()?.price | currencyBrl }}</p>
            </div>
          </div>
            ...
            <dl class="space-y-3">
              @if (dish()?.prepTimeMinutes) {
                <div class="flex justify-between">
                  ...
                  <dd class="font-medium">{{ dish()?.prepTimeMinutes }} min</dd>
                </div>
              }
              @if (dish()?.calories) {
                <div class="flex justify-between">
                  <dt class="text-gray-500">Calorias</dt>
                  <dd class="font-medium">{{ dish()?.calories }} kcal</dd>
                </div>
              }
              <div class="flex justify-between">
                <dt class="text-gray-500">Categoria</dt>
                <dd class="font-medium">{{ dish()?.categoryName }}</dd>
              </div>
              <div class="flex justify-between">
                <dt class="text-gray-500">Status</dt>
                <dd>
                  <app-badge [variant]="dish()?.active ? 'success' : 'gray'">
              ...
              <div class="flex justify-between">
                ...
                <dd class="font-medium">{{ dish()?.createdAt | date:'dd/MM/yyyy HH:mm' }}</dd>
              </div>
              @if (dish()?.updatedAt && dish()?.updatedAt !== dish()?.createdAt) {
                <div class="flex justify-between">
                  <dt class="text-gray-500">Atualizado em</dt>
                  <dd class="font-medium">{{ dish()?.updatedAt | date:'dd/MM/yyyy HH:mm' }}</dd>
                </div>
              }
            </dl>
          </div>
          ...
          <div class="card p-6">
            ...
            <div class="space-y-2">
              <app-button variant="outline" class="w-full justify-start" [routerLink]="['/dishes', dish()?.id, 'edit']">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                Editar prato
              </app-button>
...
export class DishDetailComponent implements OnInit {
  ...
  galleryImages = computed(() => 
    this.dish()?.images?.map(img => ({ url: img.imageUrl, alt: this.dish()?.name || '', primary: img.primary })) || 
    (this.dish()?.imageUrl ? [{ url: this.dish()!.imageUrl!, alt: this.dish()!.name!, primary: true }] : [])
  );
