...
@Component({
  ...
  template: `
          ...
          <div class="thumbnails" role="list" aria-label="Miniaturas">
            @for (image of images(); track image.id; let i = $index) {
              <button
                ...
                role="listitem"
                [attr.aria-label]="'Imagem ' + (i + 1) + (image.isMain ? ' (principal)' : '')"
    ...
    <!-- Fullscreen Modal -->
    @if (fullscreenOpen()) {
      <div class="fullscreen-overlay" (click)="closeFullscreen()" role="dialog" aria-modal="true" aria-label="Visualização em tela cheia">
        <button
          type="button"
