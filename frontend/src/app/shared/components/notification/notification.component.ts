...
@Component({
  ...
  template: `
    @for (notification of notifications(); track notification.id) {
      <div class="fixed top-4 right-4 z-50 flex w-full max-w-sm items-center gap-3 p-4 rounded-xl shadow-lg animate-slide-in"
        [ngClass]="getNotificationClasses(notification.type)"
        role="alert"
        aria-live="polite">
        <mat-icon class="text-current shrink-0">{{ getIcon(notification.type) }}</mat-icon>
        <p class="flex-1 text-sm font-medium text-gray-900">{{ notification.message }}</p>
        <button
          type="button"
