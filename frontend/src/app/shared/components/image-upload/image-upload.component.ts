        ...
        @if (images().length === 0) {
          <div class="drop-zone-content">
            ...
            <p class="text-sm text-gray-400 mt-1">
              JPEG, PNG, WebP • Máx. {{ maxFileSizeMB() }}MB • Até
              {{ maxFiles() }} arquivos
            </p>
          </div>
        }
          ...
          @for (image of images(); track $index) {
            ...
            >
              <div class="image-container relative">
                ...
                <!-- Actions Overlay -->
                <div
                  class="absolute inset-0 bg-black/50 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center gap-2 rounded-lg p-2"
                >
                  @if (!image.isMain) {
                    <button
                      type="button"
      ...
      for (let i = 0; i < pendingImages.length; i++) {
        ...
        try {
          ...
          const response = await this.imageUploadService
            .uploadImages(this.dishId(), new DataTransfer().files)
            .toPromise();
