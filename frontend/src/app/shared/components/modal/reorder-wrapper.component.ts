...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, DragDropModule, MatButtonModule, MatIconModule, BadgeComponent, ButtonComponent],
  template: `
    <div
...
export class ReorderWrapperComponent {
  ...
  onDrop(event: CdkDragDrop<ReorderItem[]>): void {
    ...
    this._internalItems.set(updated.map((item, index) => ({ ...item, displayOrder: index })));
  }
