...
const VARIANT_CONFIG: Record<
  ConfirmVariant,
  { icon: string; iconColor: string; confirmColor: string }
> = {
  danger: {
    icon: "warning",
...
export class ConfirmModalComponent {
  ...
  protected readonly configSignal = computed(() => ({
    ...
    closeOnBackdrop:
      this.config.baseConfig?.closeOnBackdrop ?? !this.config.loading,
    closeOnEscape:
      this.config.baseConfig?.closeOnEscape ?? !this.config.loading,
  }));
