import { Component, input, output, computed, signal, effect, inject, viewChild, ChangeDetectionStrategy } from '@angular/core';
...
export interface TableAction<T> {
  ...
  color?: 'primary' | 'secondary' | 'danger' | 'ghost' | ((row: T) => 'primary' | 'secondary' | 'danger' | 'ghost');
        ...
        <table
            ...
            <ng-container [matColumnDef]="column.key" [sticky]="column.sticky ?? false">
              ...
              <td
                ...
                *matCellDef="let row"
                ...
                [style.max-width]="column.width">
                @if (column.render) {
                  <span [class]="getCellAlign(column)">{{ column.render(row) }}</span>
                } @else {
                  <span [class]="getCellAlign(column)">{{ getCellValue(row, column.key) }}</span>
                }
              </td>
            </ng-container>
          }
          ...
          @if (actions().length > 0) {
            <ng-container matColumnDef="actions">
              <th mat-header-cell *matHeaderCellDef class="w-48 text-right pr-6">Ações</th>
          ...
          <tr class="mat-row" *matNoDataRow>
            <td [attr.colspan]="displayedColumns().length" class="text-center py-12">
              <div class="flex flex-col items-center gap-2 text-gray-500">
                <mat-icon class="text-4xl">inbox</mat-icon>
                <p>{{ config().emptyMessage ?? 'Nenhum registro encontrado' }}</p>
              </div>
            </td>
          </tr>
        </table>
...
export class TableComponent<T> {
  ...
  getActionColor(action: TableAction<T>, row: T): 'primary' | 'secondary' | 'danger' | 'ghost' {
