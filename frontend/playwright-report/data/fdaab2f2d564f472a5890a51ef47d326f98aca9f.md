...
- Name: menu.spec.ts >> Restaurante - Menu Module E2E Tests >> Menu Public View (/menu) >> should load dishes grid with cards
- Location: e2e\menu.spec.ts:93:9
...
Locator: locator('.dish-card').first()
Expected: visible
Timeout: 10000ms
Error: element(s) not found

Call log:
  - Expect "toBeVisible" locator('.dish-card').first() with timeout 10000ms
  - waiting for locator('.dish-card').first()

```

```yaml
- banner:
  - heading "Nosso Restaurante" [level=1]
  - paragraph: Cardápio Digital
  - textbox "Buscar pratos..."
  - button
- main
- contentinfo:
  - heading "Nosso Restaurante" [level=3]
  - paragraph: O melhor da culinária para você.
  - heading "Horário de Funcionamento" [level=3]
  - heading "Contato" [level=3]
  - separator
  - paragraph: © 2026 Nosso Restaurante. Todos os direitos reservados.
- heading [level=2]
- heading [level=2]
```
